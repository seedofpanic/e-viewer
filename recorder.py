...
class ScreenRecorder:
    def __init__(self):
        ...
        if self.orig_width > self.orig_height:
            ...
            self.target_width = int(self.orig_width * (self.target_height / self.orig_height))
            ...
            self.target_height = int(self.orig_height * (self.target_width / self.orig_width))
        ...
        self.fps = 10  # Target frames per second (reduced from 30 to 15 for lower resource usage)
    ...
    def _capture_loop(self):
        ...
        try:
            while self.recording:
                ...
                self.processing_futures = [f for f in self.processing_futures if not f.done()]
    ...
    def save_last_seconds(self, output_path=None):
        ...
        if output_path is None:
            ...
            output_path = f"screen_capture_{int(time.time())}.avi"  # .avi for XVID codec
    ...
    def _save_video(self, output_path):
        ...
        try:
            ...
            # Signal completion
            self.signals.processing_complete.emit({"type": "save", "path": output_path})
            return True
        except Exception as e:
    ...
    def set_buffer_size(self, seconds):
        ...
        with self.lock:
            ...
            if self.frame_buffer:
                ...
                frames_to_copy = list(self.frame_buffer)[-min(len(self.frame_buffer), new_buffer.maxlen):]
        ...
        print(f"Buffer size changed to {seconds} seconds ({self.frame_buffer.maxlen} frames)")
    ...
    def minimize_video(self, input_path, output_path=None, target_size_mb=10, maintain_aspect_ratio=True):
        ...
        if output_path is None:
            ...
            timestamp = int(time.time() * 1000)
            temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=f'_{timestamp}.avi')
        ...
        try:
            ...
            if not out.isOpened():
                ...
                out = cv2.VideoWriter(output_path, divx_fourcc, fps, (new_width, new_height))
    ...
    def save_last_seconds_minimized(self, output_path=None, target_size_mb=10):
        ...
        minimized_path = self.minimize_video(temp_output, output_path, target_size_mb)
