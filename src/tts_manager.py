...
class TTSManagerSignals(QObject):
    ...
    # Signal when a model is loaded with language code
...
class TTSManager:
    ...
    def _process_speech_queue(self):
        ...
        while True:
            try:
                ...
                # Delay to prevent tight loop if there's an error
                time.sleep(0.5)
    ...
    def _process_speech_task(self, text, rate, volume, voice, is_save, filename):
        ...
        if detected_lang and detected_lang != self._current_language:
            print(
                f"Detected language: {detected_lang}, current model language: {self._current_language}")
            print("Loading appropriate language model...")

            # Switch model in a new thread to avoid blocking
            loading_thread = threading.Thread(
                target=self.load_model, args=(detected_lang,), daemon=True)
        ...
        with self.tts_lock:
            try:
                self.play_test(text, selected_voice, speech_rate,
                               speech_volume, is_save, filename)
    ...
    def play_test(self, text, selected_voice, speech_rate, speech_volume, is_save, filename):
        ...
        if is_save and filename:
            ...
            audio = self._generate_audio(
                processed_text, selected_voice, speech_rate, speech_volume)
            if audio is None:
            ...
            batches = self._create_batches(
                sentences, self.batch_size, self.chunk_size)
            ...
            def process_batch(batch):
                ...
                chunk_audio = self._generate_audio(
                    batch_text, selected_voice, speech_rate, speech_volume)
            ...
            while chunks_played < total_chunks:
                try:
                    ...
                    # Play the chunk
                    sd.play(audio_chunk.numpy(), self.sample_rate,
                    ...
                    if all(not t.is_alive() for t in threads):
                        ...
                        if chunks_played < total_chunks:
                            print(
                                f"Warning: Only {chunks_played}/{total_chunks} chunks were processed successfully")
                        break
    ...
    def set_language(self, language_code):
        ...
        loading_thread = threading.Thread(
            target=self.load_model, args=(language_code,), daemon=True)
        loading_thread.start()
        loading_thread.join()  # Wait for model loading to complete
