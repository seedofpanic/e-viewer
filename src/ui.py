...
class ScreenRecorderUI(QMainWindow):
    def __init__(self, recorder):
        ...
        self.tts_manager.signals.speech_complete.connect(
    ...
    def update_tts_properties(self):
        ...
        current_volume = int(
            self.tts_manager.get_tts_properties().get('volume', 1.0) * 100)
        ...
        self.tts_manager.set_tts_properties(
            voice=get_prompt(selected_prompt_title)["default_voice"])
        ...
        self.voice_combo.addItem(
            TRANSLATIONS[self.current_language]["use_prompt_voice"], "default")
        ...
        self.audio_device_combo.addItem(
            TRANSLATIONS[self.current_language]["system_default"], None)
    ...
    def load_saved_settings(self):
        ...
        # Buffer size
        buffer_size = self.settings.value(
            "buffer_size", self.recorder.buffer_size, type=int)
    ...
    def initUI(self):
        ...
        language_layout.addWidget(
            QLabel(TRANSLATIONS[self.current_language]["language"]))
        ...
        self.status_label = QLabel(
            TRANSLATIONS[self.current_language]["status_ready"])
        main_layout.addWidget(self.status_label)

        # Recording controls section
        recording_section = QGroupBox(
            TRANSLATIONS[self.current_language]["recording_controls"])
        ...
        self.buffer_label = QLabel(
            TRANSLATIONS[self.current_language]["buffer_size"])
        ...
        self.buffer_size_spinner.setSuffix(
            TRANSLATIONS[self.current_language]["buffer_seconds"])
        # Use the current value from recorder
        self.buffer_size_spinner.setValue(self.recorder.buffer_size)
        self.buffer_size_spinner.setToolTip(
            "Number of seconds of footage kept in memory")
        ...
        self.start_button = QPushButton(
            TRANSLATIONS[self.current_language]["start_recording"])
        self.start_button.clicked.connect(self.start_capture)
        recording_layout.addWidget(self.start_button)

        # Stop capture button
        self.stop_button = QPushButton(
            TRANSLATIONS[self.current_language]["stop_recording"])
        ...
        # Analysis section
        analysis_section = QGroupBox(
            TRANSLATIONS[self.current_language]["analysis_controls"])
        ...
        self.prompt_label = QLabel(
            TRANSLATIONS[self.current_language]["prompt"])
        ...
        self.analyze_button = QPushButton(
            TRANSLATIONS[self.current_language]["run_analysis"])
        ...
        self.hotkey_label = QLabel(
            TRANSLATIONS[self.current_language]["hotkey"])
        ...
        self.volume_label = QLabel(
            TRANSLATIONS[self.current_language]["tts_volume"])
        volume_layout.addWidget(self.volume_label)
        self.volume_slider = QSpinBox()
        self.volume_slider.setRange(0, 100)
        self.volume_slider.setSuffix(
            TRANSLATIONS[self.current_language]["volume_percent"])
        ...
        self.speed_label = QLabel(
            TRANSLATIONS[self.current_language]["tts_speed"])
        ...
        self.speed_slider.setSuffix(
            TRANSLATIONS[self.current_language]["speed_wpm"])
        self.speed_slider.setToolTip(
            "Adjust text-to-speech speed (words per minute)")
        ...
        self.voice_label = QLabel(
            TRANSLATIONS[self.current_language]["tts_voice"])
        ...
        # API Key section
        api_section = QGroupBox(
            TRANSLATIONS[self.current_language]["gemini_api"])
        api_section.setObjectName("api_section")
        api_layout = QVBoxLayout(api_section)

        # API Key input
        self.api_key_label = QLabel(
            TRANSLATIONS[self.current_language]["api_key"])
        ...
        if self.recorder.gemini_api.api_key:
            ...
            self.api_key_input.setPlaceholderText(
                TRANSLATIONS[self.current_language]["api_key_placeholder"])

        api_layout.addWidget(self.api_key_input)
        main_layout.addWidget(api_section)

        # Results section
        results_section = QGroupBox(
            TRANSLATIONS[self.current_language]["analysis_results"])
    ...
    def stop_capture(self):
        ...
        self.status_label.setText(
            "Capture stopped. Buffer contains last recordings.")
    ...
    def update_tts_speed(self, value):
        ...
        print(
            f"TTS speed set to {value} WPM (rate multiplier: {rate_multiplier:.2f})")
    ...
    def update_analysis_result(self, result):
        ...
        print(
            f"DEBUG: Updating analysis results with text ({len(result)} chars)")
        ...
        self.analysis_result.moveCursor(
            self.analysis_result.textCursor().Start)
    ...
    def update_status(self, status):
        ...
        if status == "Ready":
            ...
            self.status_label.setText(
                TRANSLATIONS[self.current_language]["status_recording"])
        elif status == "Recording stopped.":
            self.status_label.setText(
                TRANSLATIONS[self.current_language]["status_stopped"])
        elif status == "Analyzing video...":
            self.status_label.setText(
                TRANSLATIONS[self.current_language]["status_analyzing"])
        elif status == "Analysis complete.":
            self.status_label.setText(
                TRANSLATIONS[self.current_language]["status_analysis_complete"])
        elif status.startswith("Error:"):
            error_msg = status[6:]  # Remove "Error:" prefix
            self.status_label.setText(
                TRANSLATIONS[self.current_language]["status_error"].format(error_msg))
        else:
    ...
    def update_commentary(self, commentary):
        ...
        if commentary:
            ...
            print(
                f"Commentary received and displayed ({len(commentary)} chars)")
    ...
    def update_ui_texts(self):
        ...
        self.voice_combo.setItemText(
            0, TRANSLATIONS[self.current_language]["use_prompt_voice"])

        self.audio_device_label.setText(
            TRANSLATIONS[self.current_language]["audio_device"])
        self.audio_device_combo.setItemText(
            0, TRANSLATIONS[self.current_language]["system_default"])

        # API section
        api_section = self.findChild(QGroupBox, "api_section")
        if api_section:
            api_section.setTitle(
                TRANSLATIONS[self.current_language]["gemini_api"])
