...
class ScreenRecorderApp(ScreenRecorderUI):
    def __init__(self):
        ...
        self.signals.video_analysis_status.connect(self.update_video_analysis_status)
    ...
    def create_loading_screen(self, message):
        ...
        self.splash = QDialog(None, Qt.WindowStaysOnTopHint | Qt.FramelessWindowHint)
        self.splash.resize(400, 150)
        self.splash.setStyleSheet("background-color: white; border: 1px solid #cccccc;")
        ...
        while self.tts_manager.model is None:
            ...
            if hasattr(sys.stderr, "getvalue"):
                ...
                if "error" in stderr_value.lower() or "exception" in stderr_value.lower():
                    self.on_tts_error(f"Error detected in console: {stderr_value[-200:]}")
            
            time.sleep(0.1)
            
            # Update loading message with elapsed time
            elapsed = time.time() - start_time
            self.loading_label.setText(f"Loading TTS Model... ({elapsed:.1f}s)")
            
            # If loading takes too long (over 30 seconds), break to prevent hanging
            if elapsed > 30:
                self.loading_label.setText("TTS Model load timeout. Please restart the application.")
                self.splash.setStyleSheet("background-color: #ffeeee; border: 1px solid #ff6666;")
    ...
    async def _get_gemini_response(self, video_bytes, context_id, prompt_title=None):
        ...
        if prompt_title:
            ...
            if not selected_prompt:
                print(f"Prompt with title '{prompt_title}' not found, using random prompt")
    ...
    def _handle_tts_output(self, commentary, tts_enabled, tts_volume=None, tts_rate=None, tts_voice=None):
        ...
        self.recorder.thread_pool.submit(self.tts_manager.speak_text, commentary)
        ...
        def async_runner():
            try:
                ...
                result = loop.run_until_complete(
                    self.get_sarcastic_commentary(api_key, tts_enabled=True, tts_volume=tts_volume, tts_rate=tts_rate, tts_voice=tts_voice, prompt_title=prompt_title)
                )
                
                # Get just the commentary text
                commentary = result.get("commentary", "Failed to get commentary from Gemini")

                if result.get("error", False):
                    self.signals.video_analysis_status.emit("error")
                
                # Show message
                print(f"Hotkey-triggered analysis completed in {result.get('total_time', 0):.2f} seconds")
                ...
                self.ui.signals.error_occurred.emit(f"Analysis error: {str(e)}")
            finally:
