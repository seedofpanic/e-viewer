...
class HotkeyInput(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self.listening_background = QColor(240, 240, 255)  # Light blue when listening
    ...
    def toggle_listening_mode(self):
        ...
        if self.is_listening:
            ...
            self.setText("")
            self.viewport().setStyleSheet(f"background-color: {self.listening_background.name()};")
        else:
            self.setPlaceholderText("Click here to set hotkey...")
            self.viewport().setStyleSheet(f"background-color: {self.normal_background.name()};")
